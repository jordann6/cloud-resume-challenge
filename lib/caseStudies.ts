...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    titleOut: "Dashboard",
    category: "AWS · FinOps",
    lede: "A serverless FinOps platform that answers who owns the spend, where it is wasted, and what it will be next month, not just what each service cost. Cost allocation by tag, a waste scan, reservation coverage, budgets, and anomaly detection, with least-privilege isolation at every layer.",
    meta: [
    ...
    blocks: [
      ...
      },
      {
        ...
        heading: "Outcome",
        paragraphs: [
          "Deployed 43 resources, invoked the pipeline live, and verified the new endpoints: coverage returned real RI and Savings Plans data, cost-by-tag correctly surfaced untagged spend, and the waste scan ran clean on an already-lean account. Then torn down to keep spend near zero.",
    ...
    stack: ["Lambda", "Cost Explorer", "DynamoDB", "AWS Budgets", "API Gateway", "CloudFront", "React", "EventBridge Scheduler", "Terraform"],
    repo: "https://github.com/jordann6/aws-cost-intelligence-dashboard",
    receipt: {
      rows: [
        { k: "Provision", v: "43 resources in Terraform with S3 remote backend and native state locking" },
        { k: "Demo", v: "Live cost-by-tag (untagged spend surfaced), RI/SP coverage, waste scan, and a 14-day forecast" },
  ...
  },
  {
    ...
    lede: "Cost visibility that answers what a bill cannot: which resource changed behavior, who owns the spend, where Advisor says it is wasted, and whether tags are governed at the control plane. Statistical anomaly detection, cost allocation by tag, Azure Advisor optimization, and Azure Policy tag governance over the Cost Management API, written in C# and running credential-free.",
    meta: [
      { k: "Role", v: "Cloud / FinOps" },
      { k: "Cloud", v: "Azure" },
    ...
    blocks: [
      ...
      },
      {
        ...
        paragraphs: [
          "Seven HTTP-triggered Functions expose the results as a REST API to a React single-page app on Static Web Apps. The read path never touches the Cost Management API for stored data, and cost-by-tag and Advisor queries run live on demand. The whole thing runs without a stored credential: a system-assigned managed identity holds Cost Management Reader and Reader at subscription scope (covering Advisor too) and Cosmos DB Built-in Data Contributor at database scope. Key-based Cosmos access is disabled at the account level, so even a leaked connection string would be inert.",
          "The AWS counterpart, the Cost Intelligence Dashboard, applies the same allocation, waste, and forecasting approach to Cost Explorer; running it across both clouds is what makes the practice portable. The governance layer here deployed and was demoed live, the policy assignment created, resources carrying the new cost_center and team tags, the untagged resource flagged. The Consumption Function App could not deploy in this subscription because its App Service VM quota is zero, a subscription-wide limit that needs an Azure support quota increase; the two Function-hosted features build clean and are proven end to end on the AWS side.",
        ],
      },
    ],
    stack: ["Azure Functions", "C# .NET 8", "Cosmos DB", "Cost Management API", "Azure Advisor", "Azure Policy", "Static Web Apps", "React", "Terraform"],
